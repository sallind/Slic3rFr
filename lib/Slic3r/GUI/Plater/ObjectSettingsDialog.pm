...
sub new {
    ...
    my $self = $class->SUPER::new($parent, -1, "Paramètres pour" . $params{object}->name, wxDefaultPosition, [500,500], wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
    ...
    $self->{tabpanel}->AddPage($self->{settings} = Slic3r::GUI::Plater::ObjectDialog::SettingsTab->new($self->{tabpanel}, object => $self->{object}), "Paramètres");
    $self->{tabpanel}->AddPage($self->{layers} = Slic3r::GUI::Plater::ObjectDialog::LayersTab->new($self->{tabpanel}, object => $self->{object}), "Couches");
    $self->{tabpanel}->AddPage($self->{materials} = Slic3r::GUI::Plater::ObjectDialog::MaterialsTab->new($self->{tabpanel}, object => $self->{object}), "Matériaux");
...
sub new {
    ...
    # descriptive text
    {
        ...
        my $label = Wx::StaticText->new($self, -1, "Vous pouvez utiliser cette section pour modifier quelques paramètres du tout pour cet objet.",
            wxDefaultPosition, [-1, 25]);
...
sub new {
    ...
    
    {
        ...
        #    wxDefaultPosition, [-1, 25]);
        my $label = Wx::StaticText->new($self, -1, "Vous pouvez utiliser cette section pour remplacer la hauteur de la couche par défaut pour les parties de cet objet. Définir la hauteur de la couche de zéro pour passer une portions du fichier d'entrée.",
            wxDefaultPosition, [-1, 25]);
        $label->SetFont(Wx::SystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
...
sub CanClose {
    ...
    foreach my $range ($self->_get_ranges) {
        ...
        if ($max <= $min) {
            ...
            Slic3r::GUI::show_error($self, "Plage Z invalide $min-$max.");
            return 0;
        }
        if ($min < 0 || $max < 0) {
            #Slic3r::GUI::show_error($self, "Invalid Z range $min-$max.");
        ...
        if ($height < 0) {
            ...
            Slic3r::GUI::show_error($self, "Hauteur de couche invalide $height.");
            return 0;
        }
...
sub new {
    ...
    # descriptive text
    {
        ...
        #    wxDefaultPosition, [-1, 25]);
        my $label = Wx::StaticText->new($self, -1, "Dans cette section, vous pouvez assigner des matériaux d\'objets à vos extrudeuses.",
            wxDefaultPosition, [-1, 25]);
        $label->SetFont(Wx::SystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
        $self->{sizer}->Add($label, 0, wxEXPAND | wxTOP | wxLEFT | wxRIGHT, 10);
    }
